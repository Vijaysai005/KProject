	...
	for day in range(startday,endday+1):
		for hr in range(1,24):
			for mins in range(59):
				...
				items = get_col.find({"$and" :[{"timestamp":{"$gte":str(datetime.strptime(str(hr)+":"+str(mins)+":00", '%H:%M:%S').time())}},{"timestamp":{"$lte":str(datetime.strptime(str(hr)+":"+str(mins+1)+":00", '%H:%M:%S').time())}}]},{"unit_id":1,"latitude":1,"longitude":1,"_id":0})
				...
				try:
					...
					main_dict,n_cluster = cl.cluster(data[0], data[1], 0.00045, 2)
					
					# eps = ((distance in KM / 6371)*180) / (pi) example., ((0.2/6371)*180) / (pi) = 0.0018 , for 200 metre 
					for i in range(len(main_dict)):
						try:
							for j in range(len(main_dict[i])):
								set_col.insert([{"cluster_number": i, "unit_id": int(main_dict[i][j][0]), "latitude": main_dict[i][j][1],"longitude": main_dict[i][j][2], "timestamp":ist}])
						except Exception:
							for k in range(len(main_dict["outlier"])):
								set_col.insert([{"cluster_number": "outlier", "unit_id": int(main_dict["outlier"][k][0]), "latitude": main_dict["outlier"][k][1],"longitude": main_dict["outlier"][k][2], "timestamp":ist}])
...
if __name__ == "__main__":
	...
	get_coll = db.simul_10000
	set_coll = db.clus_simul_10000

	startday = 25 ; endday = 25
	year = 2017 ; month = 3
